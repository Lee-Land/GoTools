...
func getBase64Char(code byte) byte {
	if code <= 25 { /* 大写 A - Z */
		code += 65
	} else if code > 25 && code <= 51 { /* 小写 a - z */
		code += 71
	} else if code > 51 && code <= 61 { /* 数字 0 - 9 */
		code -= 4
	} else if code == 62 {
		code = '+'
	} else if code == 63 {
...
func getBase64Code(char byte) (byte, error) {
	...
	if char >= 48 && char <= 57 { /* 数字 0 - 9 */
		...
		return char, fmt.Errorf("invalid utils text format")
	}

	return char, nil
}

// Encode 编码, src 为字节数组, end 为结束标记
